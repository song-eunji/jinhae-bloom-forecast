...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      h("1. 분석 개요", HeadingLevel.HEADING_2),
      ...
      h("2. 분석 결과 상세 내용", HeadingLevel.HEADING_2),
      ...
      h("3. 결과 해석 및 시사점", HeadingLevel.HEADING_2),
      ...
      h("4. 활용방안 및 기대효과", HeadingLevel.HEADING_2),
      ...
      p("같은 페이지에 '이번 시즌 예측(나우캐스팅)' 기능도 함께 제공한다. 축제 담당자가 날짜별 실측 평균기온을 직접 입력하거나 CSV 파일로 일괄 업로드하면, 입력된 날짜는 실측값을, 입력되지 않은 날짜(시작일과 마지막 입력일 사이의 공백 및 그 이후 구간)는 2000~2026년 같은 날짜의 평년 기온으로 채워 적산온도를 끝까지 계산하고 예상 개화·만발일을 산출한다. 과거 연도의 실측 일별 기온을 그대로 입력값으로 대입해 역으로 검증한 결과, 입력 구간이 3월 초까지 늘어나면 예측오차가 대부분 0~2일 수준으로 좁혀졌다. 시즌이 진행되며 입력 데이터가 누적될수록 예측은 계속 갱신·개선되므로, 담당자가 주기적으로 최신 기온을 추가하며 축제 일정 검토에 참고할 수 있다."),
      p("데모 주소: https://claude.ai/code/artifact/df99c81e-6133-4a7e-8a32-8e529082a427", { bold: true, color: "7A3654" }),
      ...
      h("5. 활용데이터 및 참고문헌 출처", HeadingLevel.HEADING_2),
...
Packer.toBuffer(doc).then((buf) => {
  fs.writeFileSync("진해벚꽃_개화예측_분석보고서_v6.docx", buf);
  console.log("written");
});
